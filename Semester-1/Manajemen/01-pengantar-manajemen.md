...
```mermaid
flowchart LR
A[Organisasi] --> B[Manajer]
B --> C[Proses Manajemen]
C --> D[Teori Manajemen]
D --> E[Tantangan Masa Depan]
```
...
```mermaid
flowchart TD
A[Tujuan Besar] --> B[Organisasi]
B --> C[Manajer]
C --> D[Fungsi Manajemen]
D --> E[Peranan Manajerial]
E --> F[Teori Manajemen]
F --> G[Tantangan Modern]
```
...
```mermaid
flowchart TD
A[Keterbatasan Manusia] --> B[Tujuan Besar Tidak Bisa Dicapai Sendiri]
B --> C[Butuh Organisasi]
C --> D[Butuh Manajemen]
D --> E[Koordinasi]
E --> F[Tujuan Tercapai]
```
